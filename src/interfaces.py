from agent_LLM import _generate_post_LLM, _like_decision_LLM, _generate_post_warmup_LLM, _initialize_agents_LLM
...
def _generate_post(agent, timestep, POSTS, PARAMS):
    if PARAMS["opinion_model"] == "LLM":
        return _generate_post_LLM(agent, timestep, POSTS, PARAMS)
    else:
...
def _like_decision(agent, timestep, POSTS, post, PARAMS):
    if PARAMS["opinion_model"] == "LLM":
        return _like_decision_LLM(agent, timestep, POSTS, post, PARAMS)
    else:
        print("unknown opinion model")
        exit()
...
def _generate_post_warmup(agent, timestep, POSTS, PARAMS):
    if PARAMS["opinion_model"] == "LLM":
        return _generate_post_warmup_LLM(agent, timestep, POSTS, PARAMS)
    else:
        print("unknown opinion model")
        exit()
